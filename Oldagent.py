...
class Bauerle_Rieder_agent(object):
    ...
    def value_iteration(self,utility_function='risk-neutral',save_results=True,load_results=True,depth_and_chunks=None):
        ...
        if load_results:
            vr_dir=os.path.join(os.getcwd(),'Old_Value_iteration_results')
                ...
                for val in cost_reward_values:
                    ...
                    for to_add_val in tmp:
                        if len(temp_values)==0:
                            temp_values=np.append(temp_values,to_add_val)
                        else:
                            is_redundant=(sum(abs(temp_values-to_add_val)<1.0e-3)>0)
                            if is_redundant==False:
                                temp_values=np.append(temp_values,to_add_val)
                for to_add_val2 in temp_values:
                    is_redundant=(sum(abs(values-to_add_val2)<1.0e-3)>0)
                    if is_redundant==False:
                                values=np.append(values,to_add_val2)
    ...
    def mu_to_index(self,mu):
        ...
        path, dirs, files = next(os.walk("./Old_Universal_Mu_chunks"))
